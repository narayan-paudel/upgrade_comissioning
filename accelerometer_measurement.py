...
for irotation in range(0,361,10)[:]:
    ...
    if val == "y":
        with open('/Users/epaudel/research_ua/icecube/upgrade/upgrade_comissioning/data/sensor_data/mDOMMB_accelerometer_magnetometerMay16.txt', 'a+') as fh1:
            for i in range(1,11):
                gx,gy,gz = session.readAccelerometerXYZ()
                bx,by,bz = session.readMagnetometerXYZ()
        ...
        if s == "y":
            ...
            with open('/Users/epaudel/research_ua/icecube/upgrade/upgrade_comissioning/data/sensor_data/mMB_accelerometer_magnetometerMay16.txt', 'a+') as fh2:
                for i in range(1,11):
                    gx,gy,gz = session.readAccelerometerXYZ()
                    bx,by,bz = session.readMagnetometerXYZ()
                    fh2.write(f"{irotation} {gx} {gy} {gz} {bx} {by} {bz}\n")
        input("did you switch back the main board and rotate, input y")
    else:
        break        
